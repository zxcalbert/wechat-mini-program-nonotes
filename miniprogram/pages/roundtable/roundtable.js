...
Page({
    ...
    wx.cloud.callFunction({
      ...
      success: (res) => {
        if (res.result.success) {
          ...
          this.setData({ methodRules: rules });
  ...
  fallbackToHardcoded() {
    ...
    this.setData({
      fallbackMethodGuides: methodGuides
    });
  },
  ...
  updateCanSubmit() {
    const selectedCount = this.data.selectedMethods.length;
  ...
  toggleMethod(e) {
    ...
    this.setData({ 
      ...
      selectedMethodsMap: selectedMethodsMap
    }, () => {
  ...
  async submitRoundtable() {
    ...
    if (this.data.selectedMethods.length < 3) {
      wx.showToast({
        title: '请至少选择3种分析方法',
        icon: 'none',
        duration: 2000
