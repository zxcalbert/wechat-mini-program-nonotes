...
### 1.1 项目定位
智慧笔记是一款基于微信小程序的个人思考记录工具，用户可以记录自己的感悟，选择不同的AI人生导师（查理·芒格、巴菲特、段永平、张小龙、乔布斯、马斯克），获取模拟的智慧回复，帮助用户深入反思和思考。
...
| 首页 | `/pages/index/` | 笔记列表展示、搜索、分页加载、侧边菜单 |
...
| AI回复生成 | `/cloudfunctions/replyToLetter/` | 调用DeepSeek API生成导师回复、敏感词检测 |
...
2. 创建云开发环境，记录环境ID
3. 在云开发控制台创建三个集合：`users`、`letters`、`stampHistory`，权限均设置为"仅创建者可读写"
...
### Q3: 邮票机制是怎样的？
A: 新用户默认赠送10张邮票，每生成一次AI回复消耗1张邮票，用户可以在邮票页面购买更多邮票套餐。

### Q4: 回复为什么需要18小时才能查看？
A: 这是产品设计特性，目的是鼓励用户在写下笔记后先自行思考，延迟满足，避免过度依赖AI回复。

### Q5: 用户数据是如何隔离的？
A: 云数据库权限设置为"仅创建者可读写"，每个用户只能访问自己创建的笔记和数据，云函数操作也会自动校验用户openid。
