...
### 13.1 测试策略：Plan模式主体 + Spec优势融合

> 📌 **核心原则**：保持Plan模式的系统性和结构性优势，同时有机融合Spec模式的行为描述特性

本项目采用**渐进式测试策略**，以Plan模式为主体框架，融合Spec模式的优势特性：

| 维度 | Plan模式（主体） | Spec模式（融合优势） |
|------|------------------|---------------------|
| 测试组织 | 按功能模块和开发阶段组织 | 按用户行为和场景描述 |
| 验收标准 | 明确的任务完成标准 | 详细的行为预期描述 |
| 测试粒度 | 模块级功能验证 | 场景级端到端验证 |
| 覆盖重点 | 核心业务逻辑 | 用户体验和边界场景 |

#### 13.1.1 选择性单元测试原则

**适用场景**：仅对核心算法和关键业务逻辑实施单元测试

**筛选标准**：
| 标准 | 说明 | 示例 |
|------|------|------|
| 🔴 **核心算法** | 涉及数据处理、计算逻辑的独立函数 | 字数统计、敏感词检测、内容截断 |
| 🔴 **业务规则** | 决定业务走向的关键判断逻辑 | 权限判定、积分计算、状态流转 |
| 🟡 **复用组件** | 被多处引用的工具函数 | 日期格式化、数据转换工具 |
| 🟢 **UI逻辑** | 纯展示和交互逻辑 | 样式计算、动画控制 |

**豁免场景**：
- 页面级事件处理函数（除非包含复杂业务逻辑）
- UI组件内部状态管理
- 简单的数据绑定和渲染逻辑
...
| 类型 | 说明 | 优先级 | 实施方式 |
|------|------|--------|----------|
| 功能测试 | 核心功能验证 | 🔴 高 | 手动测试 + 自动化检查 |
| 兼容性测试 | 不同机型、系统版本 | 🟡 中 | 手动测试 |
| 性能测试 | 启动速度、内存占用 | 🟡 中 | 手动测试 |
| 安全测试 | 数据隔离、权限验证 | 🔴 高 | 代码审查 + 手动测试 |
| 单元测试 | 核心算法验证 | 🟡 中 | 自动化脚本 |
...
> 📌 **设计原则**：
> 1. **工具模块与测试用例分离**：核心算法工具独立，测试用例按模块组织
> 2. **通用测试框架**：提供可复用的测试基础功能（test、assert、assertEqual等）
> 3. **阶段性测试模块**：按开发阶段组织专项测试，但不将具体阶段名称固化到通用架构中
> 4. **渐进式扩展**：新模块添加时，遵循现有结构独立测试

```
scripts/
└── tests/
    ├── wordCountUtil.js          # 字数统计工具模块（核心算法）
    ├── sensitiveWordUtil.js      # 敏感词检测工具模块（核心算法）
    ├── testFramework.js          # 通用测试框架（如需要独立）
    └── integration-tests.js      # 集成测试入口（通用名称，不绑定具体阶段）
```
...
| 模块类型 | 说明 | 适用范围 | 复用策略 |
|---------|------|---------|---------|
| **工具模块** | 包含核心算法的独立文件 | 所有需要该算法的阶段 | 永久复用，与主代码保持同步 |
| **测试入口模块** | 整合多个工具模块的测试用例 | 当前开发阶段 | 可演进，按需要添加新测试 |
| **通用测试框架** | test、assert等基础测试函数 | 所有测试 | 全局复用，禁止修改 |
...
test('空字符串处理', () => {
  const text = '';
  assertEqual(countChineseWords(text), 0, '空字符串应返回0');
});

// 异常测试：异常输入
test('特殊字符处理', () => {
  const text = '测试🎉emoji';
  assertEqual(countChineseWords(text), 3, '应正确处理emoji');
});
```
...
# 运行特定模块测试（示例）
node scripts/tests/wordCountUtil.js
```

> 📌 **注意**：
> - `integration-tests.js` 为通用名称，可根据当前开发阶段需要包含相应测试用例
> - 具体的阶段性测试（如Phase 2专项测试）属于临时测试文件，不应固化到项目规则中
> - 工具模块测试可独立运行，便于快速验证核心算法变更
